...
class ImagesVectorDB {
    ...
    // Return the top-N best-matching persons for a given query embedding.
    // Groups by personID, keeps the best (highest cosine similarity) embedding per person,
    // filters by threshold, then returns the top N sorted descending.
    fun getTopNCandidates(
        embedding: FloatArray,
        ...
        // Map personID -> best cosine similarity across all stored face embeddings
        val bestPerPerson = HashMap<Long, Float>()
        for (record in allRecords) {
            val sim = cosineDistance(embedding, record.faceEmbedding)
            val current = bestPerPerson[record.personID]
            if (current == null || sim > current) {
                bestPerPerson[record.personID] = sim
            }
        }

        // Build a lookup from personID to PersonRecord for metadata
        val personMap = personRecords.associateBy { it.personID }

        return bestPerPerson.entries
            .filter { (_, sim) -> sim >= threshold }
            .sortedByDescending { (_, sim) -> sim }
            .take(topN)
            .mapNotNull { (personID, sim) ->
                val person = personMap[personID] ?: return@mapNotNull null
                MatchCandidate(
                    personID = personID,
